...
public class XRefreshView extends LinearLayout {
	 ...
	 */
	private int mHeaderPinnedTime = 3000;
	private int mBottomPinnedTime = 1000;
	...
	private void addHeaderView() {
		if (mHeaderView == null) {
//			mHeaderView = new JumpLoadingView(getContext());
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			int childWidthSpec = MeasureSpec.makeMeasureSpec(
					width - lp.leftMargin - lp.rightMargin - paddingLeft - paddingRight, MeasureSpec.EXACTLY);
			// int childWidthSpec = getChildMeasureSpec(widthMeasureSpec,
			// paddingLeft + paddingRight, getMeasuredWidth()-lp.leftMargin -
			// lp.rightMargin);
...
//		LogUtils.d("onLayout mHolder.mOffsetY=" + mHolder.mOffsetY);
	 ...
	 * 设置是否跳过了按下事件
	 * 
	 * @param is_jump_DownEvent
	 */
	public void setIs_jump_DownEvent(boolean is_jump_DownEvent) {
		this.is_jump_DownEvent = is_jump_DownEvent;
		this.jumped_down_count = 0;
	}
...
//			LogUtils.d("isTop=" + mContentView.isTop() + ";isBottom=" + mContentView.isBottom());
			...
			if (!mPullLoading && !mReleaseToLoadMore && mContentView.isTop()
					&& ((deltaY > 0 && !mHolder.hasFooterPullUp()) || (deltaY < 0 && mHolder.hasHeaderPullDown()))) {
				sendCancelEvent();
		...
		case MotionEvent.ACTION_UP:
			...
			if (mHolder.hasHeaderPullDown()) {
				if (mEnablePullRefresh && !mStopingRefresh && !mPullRefreshing
	...
	public enum  AppbarLayoutStatus{
		isWholeOpenStatus,isOther,isWholeOpenClose;
	}

	private AppbarLayoutStatus currentAppbarStatus;
	public void disallowInterceptTouchEvent(boolean isIntercept,AppbarLayoutStatus status) {
		mIsIntercept = isIntercept;
		currentAppbarStatus = status;
	}
...
//			LogUtils.d("sendDownEvent");
	 ...
	 *
	 * @param moveHeadWhenDisablePullRefresh
	 *            默认是true
	 */
	 ...
	 *
	 * @param moveFootWhenDisablePullLoadMore
	 *            默认为true
	 */
	...
	private boolean canReleaseToLoadMore() {
		return enableReleaseToLoadMore && mEnablePullLoad && mContentView != null && !mContentView.hasLoadCompleted()
	...
	private void updateFooterHeight(int deltaY) {
		...
		if (needAddFooterView() || enableRecyclerViewPullUp) {
			if (mEnablePullUpWhenLoadCompleted || !mContentView.hasLoadCompleted()) {
				if (mContentView.hasLoadCompleted() && needAddFooterView() && mFooterCallBack != null
						&& mFooterCallBack.isShowing()) {
					mFooterCallBack.show(false);
				}
				/*
				 * if (!needAddFooterView() && mContentView.getState() !=
				 * XRefreshViewState.STATE_COMPLETE && autoLoadMore) {
				 * //当时是recyclerview，自动加载更多，并且没有加载完全的时候，不让Recyclerview上拉 } else
				 */
	...
	public void startRefresh() {
		if (!mEnablePullRefresh || mHolder.mOffsetY != 0 || mContentView.isLoading() || mPullRefreshing
				|| !isEnabled()) {
			return;
		}
		if (mLayoutReady) {
...
//		LogUtils.d("resetHeaderHeight offsetY=" + offsetY);
	...
	public void moveView(int deltaY) {
		...
		if (mRefreshViewListener != null && (mContentView.isTop() || mPullRefreshing)) {
			...
			// headerMovePercent = headerMovePercent > 1 ? 1 :
			// headerMovePercent;
	...
	private void stopLoadMore(final boolean hideFooter, final int scrollBackDuration) {
		if (needAddFooterView()) {
			if (mPullLoading) {
				...
				if (mBottomPinnedTime >= 1000) {// 在加载更多完成以后，只有mPinnedTime大于1s才生效，不然效果不好
					postDelayed(new Runnable() {
...
//				LogUtils.d("currentY=" + currentY + ";mHolder.mOffsetY=" + mHolder.mOffsetY);
	...
	public void setEmptyView(@LayoutRes int emptyView) {
		...
		if (!resourceTypeName.contains("layout")) {
			throw new RuntimeException(getContext().getResources().getResourceName(emptyView)
...
//		mContentView.setPinnedTime(pinnedTime);
	 ...
	 *
	 * @param hide
	 *            true则隐藏footerview，false则反之，默认隐藏
	 */
	...
	public interface XRefreshViewListener {
		...
		/**
		 * @param isPullDown
		 *            是不是由下拉手势引起的刷新，是则返回true，反之则是自动刷新或者是调用
		 *            {@link #startRefresh()}引起的刷新
		 */
